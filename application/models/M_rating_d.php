...
class M_rating_d extends CI_Model {
	...
	function getData()
	{
		$query = $this->db->query("SELECT * FROM rating_d ORDER BY id_rating DESC");
		if ($query->num_rows() > 0) {
			return $query->result();
		}else {
			return array();
	...
	function insertData()
	{
		...
		return $this->db->insert('rating_d', $data);
	}
